...
outCSV = pd.DataFrame(columns=('compound', 'concentration', 'plate',
                               'well', 'replicate'))
...
f = h5py.File(os.path.join(proj_root, "input/processed/full_images_all.hdf5"), "w")
...
for row in zip(combined['Image_FileName_DAPI'],
               ...
               combined['Image_Metadata_Compound'],
               combined['Image_Metadata_Concentration'],
               combined['Image_Metadata_Plate_DAPI'],
    ...
    outCSV.loc[len(outCSV)] = [compound, concentration,
...
outCSV.to_csv(os.path.join(proj_root, 'input/processed/full_images_all.csv'))
pbar.close()
