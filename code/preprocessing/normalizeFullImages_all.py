...
info_csv = pd.read_csv(os.path.join(processed_path, 'full_images_all.csv'))

images = h5py.File(os.path.join(processed_path,
                                "full_images_all.hdf5"), "r")['images']

f = h5py.File(os.path.join(processed_path, "full_images_all_illumcorr.hdf5"), "w")
corr_images = f.create_dataset("images", images.shape)

pbar = tqdm(total=len(images))
maxima = np.zeros((len(images)))
minima = np.zeros((len(images)))
